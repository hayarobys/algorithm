...
class Problem_1032 {
	...
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in)) {
			...
			ArrayList<String> fileList = new ArrayList<String>(line);
			for(int i=0; i<line; i++){
				fileList.add(sc.nextLine());
			}
			
			System.out.println(analyze(fileList));
		}
	}
	
	public static String analyze(ArrayList<String> list){
		StringBuilder pattern = new StringBuilder();
		String target = list.get(0);
		int loopCount = target.length();
		for(int i=0; i<loopCount; i++){
			boolean allSame = true;
			String targetC = target.substring(i, i+1);
			for(String fileName:list){
				if(targetC.equals( fileName.substring(i, i+1) ) == false){
					allSame = false;
					break;
				}
			}
			pattern.append(allSame ? targetC : "?");
		}
