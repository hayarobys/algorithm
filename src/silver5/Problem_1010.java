...
class Problem_1010 {
	...
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in)) {
			...
			List<String> caseList = new ArrayList<String>(count);
			for(int i=0; i<count; i++){
				caseList.add(sc.nextLine());
			}
			
			// M개 중에 N개를 뽑는 경우의 수 M! / ((M-N)! * N!)
			for(String e:caseList){
				String[] nmArray = e.split(" ");
				BigDecimal n = new BigDecimal(nmArray[0]);
				BigDecimal m = new BigDecimal(nmArray[1]);
				System.out.println(
						factorial(m).divide(
								factorial( m.subtract(n) ).multiply( factorial(n) )
						)
				);
			}	
		}
	...
	public static BigDecimal factorial(BigDecimal n){
		...
		for(int i=2; i<=n.intValue(); i++){
			fact = fact.multiply(new BigDecimal(Integer.toString(i)));
		}
		return fact;
	}
}
